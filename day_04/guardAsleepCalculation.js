...
function guardAsleepCalculation (guardSleepingLogStrings) {
  ...
  const highestSleepingGuard = findSleepiestGuard(guardSleepLogs)

  // find the minute w/ most asleep times for the highest amount of sleep guard
  const minuteMostAsleep = findMinuteGuardMostSleepsOn(highestSleepingGuard)

  // multiply the guard id with the minute mentioned in previous step
  return highestSleepingGuard.guardId * minuteMostAsleep
}
...
function calculateAmountOfSleep (guardLog) {
  const sleepAmount = guardLog.sleepSchedule
    .reduce((total, sleepEntry) => {
      const {start, end} = sleepEntry
...
function findSleepiestGuard (guardLogs) {
  const sortedGuardsAndSleepAmounts = guardLogs
    .map(guardLog => {
      return {
        guardId: guardLog.guardId,
        sleepAmount: calculateAmountOfSleep(guardLog)
      }
    })
    .slice() // avoid sort in place
    .sort((guardLog1, guardLog2) => {
      return guardLog1.sleepAmount - guardLog2.sleepAmount
    })

  const highestSleepingEntry = last(sortedGuardsAndSleepAmounts)

  return guardLogs.find(guardLog => guardLog.guardId === highestSleepingEntry.guardId)
}

function findMinuteGuardMostSleepsOn (guardLog) {
  const { sleepSchedule } = guardLog
  const sleepRanges = sleepSchedule
